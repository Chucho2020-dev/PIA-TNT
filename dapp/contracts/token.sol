...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    constructor (string memory name_, string memory symbol_, uint256 initialBalance_, uint256 decimals_, uint256 price_) {
        _name = name_;
        _symbol = symbol_;
   ...
   function setActive(bool isActive_) public returns (bool) {
   ...
   function presale(uint256 amount_) public payable returns (bool) {
        require(amount_ <= msg.value, "The amount doesn't match!");
   ...
   function setReceiverAddress(address addr_) public returns (bool) {
        require (_owner == msg.sender, "Only the owner of the contract can do this action!");
        _receiverAddress = addr_;
        return true;
   }
   ...
   function setPrice(uint256 price_) public returns (bool) {
        require (_owner == msg.sender, "Only the owner of the contract can do this action!");
        _price = price_;
        return true;
   }
...
contract MyToken is ERC20 {
    constructor(
        ...
        uint256 price_
    ) payable ERC20(name_, symbol_, initialBalance_, decimals_, price_) {
